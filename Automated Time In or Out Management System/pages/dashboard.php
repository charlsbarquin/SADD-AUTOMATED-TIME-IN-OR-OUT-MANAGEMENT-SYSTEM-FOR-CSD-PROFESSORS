...
// Function to check if professor is late and format the time
function checkLateStatus($checkInTime, $scheduledTime)
{
    if (!$checkInTime || !$scheduledTime) return ['status' => '', 'minutes' => 0, 'formatted' => ''];
    ...
    if ($checkIn > $scheduled) {
        ...
        // Format the late time for display
        if ($minutesLate < 60) {
            $formatted = $minutesLate . ' min';
        } else {
            $hours = floor($minutesLate / 60);
            $remainingMinutes = $minutesLate % 60;
        ...
        // Format early time if needed (optional)
        $formatted = $minutesEarly . ' min';
        if ($minutesEarly >= 60) {
            $hours = floor($minutesEarly / 60);
            $remainingMinutes = $minutesEarly % 60;
            $formatted = $hours . ' hr' . ($hours != 1 ? 's' : '');
            if ($remainingMinutes > 0) {
                $formatted .= ' ' . $remainingMinutes . ' min';
            }
