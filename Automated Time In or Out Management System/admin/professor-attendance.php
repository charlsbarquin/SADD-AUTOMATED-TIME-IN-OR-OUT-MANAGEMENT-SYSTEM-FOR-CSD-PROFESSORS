...
$stats_query = "SELECT 
                  ...
                  SUM(CASE WHEN status = 'half-day' THEN 1 ELSE 0 END) as late,
                                ...
                                <?php if ($attendance_data->num_rows > 0): ?>
                                    <?php while ($record = $attendance_data->fetch_assoc()): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php 
                                                // Determine which check-in to show (AM or PM)
                                                $check_in = $record['am_check_in'] ?: $record['pm_check_in'];
                                                if ($check_in): ?>
                                                    <?= date('h:i A', strtotime($check_in)) ?>
                                                <?php else: ?>
                                                    N/A
                                                <?php endif; ?>
                                            </td>
                                            <td>
                                                <?php 
                                                // Determine which check-out to show (AM or PM)
                                                $check_out = $record['pm_check_out'] ?: $record['am_check_out'];
                                                if ($check_out): ?>
                                                    <?= date('h:i A', strtotime($check_out)) ?>
                                                <?php else: ?>
                                            ...
                                            <td>
                                                <?php 
                                                $duration = 0;
                                                if ($record['am_check_in'] && $record['am_check_out']) {
                                                    $duration += $record['am_duration'];
                                                }
                                                if ($record['pm_check_in'] && $record['pm_check_out']) {
                                                    $duration += $record['pm_duration'];
                                                }
