...
// Process attendance if form submitted
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['record_attendance'])) {
    $professor_id = $_POST['professor_id'];
    $checkin_type = $_POST['checkin_type']; // 'am' or 'pm'
    $checkin_time = date('Y-m-d H:i:s');

    // First check if record exists for today
    $check_query = $conn->prepare("SELECT id FROM attendance WHERE professor_id = ? AND checkin_date = CURDATE()");
    $check_query->bind_param("i", $professor_id);
    $check_query->execute();
    ...
    $schedule_query = $conn->prepare("
        ...
        WHERE professor_id = ? AND day_id = DAYOFWEEK(CURDATE())
    ");
    $schedule_query->bind_param("i", $professor_id);
    $schedule_query->execute();
    $schedule = $schedule_query->get_result()->fetch_assoc();

    if ($schedule) {
        $scheduled_time = strtotime($schedule['start_time']);
        $actual_time = strtotime(date('H:i:s', strtotime($checkin_time)));
    ...
    if ($exists) {
        ...
        $column = $checkin_type . '_check_in';
        $update = $conn->prepare("UPDATE attendance SET $column = ?, is_late = ? WHERE id = ?");
        $update->bind_param("sii", $checkin_time, $is_late, $exists['id']);
        $update->execute();
    } else {
        // Create new record
        $column = $checkin_type . '_check_in';
        $insert = $conn->prepare("INSERT INTO attendance (professor_id, checkin_date, $column, is_late) VALUES (?, CURDATE(), ?, ?)");
        $insert->bind_param("isi", $professor_id, $checkin_time, $is_late);
        $insert->execute();
    }
...
// Dashboard Statistics - Add late_arrivals_today
$stats = [
    ...
    'late_arrivals_today' => $conn->query("SELECT COUNT(*) FROM attendance WHERE checkin_date = CURDATE() AND is_late = 1")->fetch_row()[0],
];
...
$late_arrivals = $conn->query("
    SELECT 
        ...
        CASE
            ...
            ELSE TIMESTAMPDIFF(MINUTE, ps.start_time, 
                CASE 
                    WHEN a.am_check_in IS NOT NULL THEN TIME(a.am_check_in)
                    WHEN a.pm_check_in IS NOT NULL THEN TIME(a.pm_check_in)
                END)
    ...
    LEFT JOIN attendance a ON a.professor_id = p.id 
        AND a.checkin_date = '$today'
        AND (
            (a.am_check_in IS NOT NULL AND TIME(a.am_check_in) BETWEEN ps.start_time AND ps.end_time)
            OR
            (a.pm_check_in IS NOT NULL AND TIME(a.pm_check_in) BETWEEN ps.start_time AND ps.end_time)
        )
    WHERE ps.day_id = $current_day_id
    AND (
        (a.id IS NOT NULL AND (
            (a.am_check_in IS NOT NULL AND TIME(a.am_check_in) > ps.start_time)
            OR 
...
$on_time_professors = $conn->query("
    SELECT 
        ...
        p.department,
        p.designation,
        TIME(a.am_check_in) as checkin_time,
        'AM' as session,
        ps.start_time as scheduled_time,
        ps.end_time,
        ps.subject,
        ps.room,
        TIMESTAMPDIFF(MINUTE, TIME(a.am_check_in), ps.start_time) as minutes_early
    FROM professor_schedules ps
    JOIN professors p ON ps.professor_id = p.id
    JOIN attendance a ON a.professor_id = p.id 
        AND a.checkin_date = '$today'
        AND a.am_check_in IS NOT NULL
    WHERE ps.day_id = $current_day_id
    AND TIME(a.am_check_in) <= ps.start_time
    ...
    SELECT 
        ...
        p.department,
        p.designation,
        TIME(a.pm_check_in) as checkin_time,
        'PM' as session,
        ps.start_time as scheduled_time,
        ps.end_time,
        ps.subject,
        ps.room,
        TIMESTAMPDIFF(MINUTE, TIME(a.pm_check_in), ps.start_time) as minutes_early
    FROM professor_schedules ps
    JOIN professors p ON ps.professor_id = p.id
    JOIN attendance a ON a.professor_id = p.id 
        AND a.checkin_date = '$today'
        AND a.pm_check_in IS NOT NULL
    WHERE ps.day_id = $current_day_id
    AND TIME(a.pm_check_in) <= ps.start_time
...
// Recent Notifications - Excluding chatbot notifications
$notifications = $conn->query("
    SELECT * FROM logs 
    WHERE action NOT LIKE '%chatbot%' 
...
// Process PDF generation if form submitted
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['generate_pdf'])) {
    ...
    $stmt = $conn->prepare("
        ...
        ORDER BY a.checkin_date, p.name
    ");
    ...
    $pdf->SetCreator('Bicol University Polangui');
    $pdf->SetAuthor('Attendance System');
    $pdf->SetTitle('Attendance Report - ' . $start_date . ' to ' . $end_date);
    $pdf->SetSubject('Attendance Report');

    // Set default header data
    $pdf->SetHeaderData('', 0, 'Bicol University Polangui', 'Attendance Report - ' . date('F j, Y', strtotime($start_date)) . ' to ' . date('F j, Y', strtotime($end_date)));
    ...
    <p><strong>Total Records:</strong> ' . count($attendance_data) . '</p>
    ...
    foreach ($attendance_data as $record) {
        $html .= '
            <tr>
                <td>' . date('m/d/Y', strtotime($record['checkin_date'])) . '</td>
                <td>' . htmlspecialchars($record['name']) . '</td>
                <td>' . htmlspecialchars($record['department']) . '</td>
                <td>' . htmlspecialchars($record['designation']) . '</td>
                <td>' . ($record['am_check_in'] ? date('h:i A', strtotime($record['am_check_in'])) : '-') . '</td>
                <td>' . ($record['pm_check_in'] ? date('h:i A', strtotime($record['pm_check_in'])) : '-') . '</td>
                <td>' . ($record['pm_check_out'] ? date('h:i A', strtotime($record['pm_check_out'])) : '-') . '</td>
                <td>' . $record['status'] . '</td>
            </tr>';
    }
