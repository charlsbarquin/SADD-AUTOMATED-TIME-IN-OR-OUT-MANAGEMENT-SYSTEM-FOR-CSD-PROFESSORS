...
// Get accurate late arrivals count by checking against schedules
$accurate_late_arrivals = $conn->query("
    SELECT COUNT(DISTINCT a.id) as late_count
    FROM attendance a
    JOIN professor_schedules ps ON 
        a.professor_id = ps.professor_id AND 
        DAYOFWEEK(a.checkin_date) = ps.day_id
    WHERE DATE(a.checkin_date) BETWEEN '$start_date' AND '$end_date'
    AND (
...
// Update the stats with accurate late count
$attendance_stats['late_arrivals'] = $accurate_late_arrivals['late_count'] ?? 0;
...
// Get late arrivals for the card
$late_arrivals_card = $conn->query("
    SELECT COUNT(DISTINCT a.id) as late_count
    FROM attendance a
    WHERE a.is_late = 1
    AND DATE(a.checkin_date) BETWEEN '$start_date' AND '$end_date'
")->fetch_assoc();
?>
<!DOCTYPE html>
<html lang="en">
                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    ...
                                    <h3 class="stat-value"><?= $late_arrivals_card['late_count'] ?? 0 ?></h3>
                            ...
                            <tbody>
                                <?php if ($report_data->num_rows > 0): ?>
                                    ...
                                    <?php while ($row = $report_data->fetch_assoc()): ?>
                                        ...
                                        <?php elseif ($report_type === 'late_arrivals'): ?>
                                            <tr>
                                                <td><?= htmlspecialchars($row['name'] ?? 'N/A') ?></td>
                                                <td><?= htmlspecialchars($row['department'] ?? 'N/A') ?></td>
                                                <td><?= !empty($row['date']) ? date('M j, Y', strtotime($row['date'])) : 'N/A' ?></td>
                                                <td><?= !empty($row['scheduled_time']) ? date('h:i A', strtotime($row['scheduled_time'])) : 'N/A' ?></td>
                                                <td><?= !empty($row['checkin_time']) ? date('h:i A', strtotime($row['checkin_time'])) : 'N/A' ?></td>
                                                <td><?= $row['minutes_late'] ?? 'N/A' ?></td>
                                                <td>
                                                    <span class="badge <?= $row['status'] === 'Active' ? 'bg-info' : 'bg-secondary' ?>">
                                                        <?= $row['session'] ?? 'N/A' ?>
                                                    </span>
                                                </td>
                                                ...
                                                <td>
                                                    <span class="badge <?= $row['status'] === 'Active' ? 'bg-warning' : 'bg-success' ?>">
                                                        <?= $row['status'] ?? 'N/A' ?>
                                                    </span>
                                                </td>
                                            </tr>
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="<?=
                                                        $report_type === 'daily_summary' ? 7 : ($report_type === 'professor_activity' ? 8 : ($report_type === 'attendance_details' ? 8 : ($report_type === 'late_arrivals' ? 10 : 7)))
